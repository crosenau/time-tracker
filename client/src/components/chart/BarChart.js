...
class BarChart extends Component {
  ...
  drawChart() {
    ...
    const rem = smallScreen ? 
      parseFloat(getComputedStyle(document.documentElement).fontSize) * 0.75 :
      parseFloat(getComputedStyle(document.documentElement).fontSize);
    const widescreen = width > height * 1.3;
    const maxLabelLength = 16; // Max character length of tasks

    const margin = {
      top: 0,
      right: widescreen ? rem * maxLabelLength + rem : 2 * rem,
      bottom: widescreen ? 2 * rem : height / 2,
      left: 2 * rem
    };
    ...
    if (widescreen) {
      ...
      legendGroups.append('rect')
        ...
        .attr('y', (d, i) => 2 * rem + height / 2 + Math.floor(i / 2) * (2 * rem))
        .attr('width', 1.5 * rem)
        .attr('height', 1.5 * rem)
        .attr('fill', d => colorScale(taskLabels.indexOf(d[0])));
      ...
      legendGroups.append('text')
        ...
        .attr('y', (d, i) => 3.2 * rem + height / 2 + Math.floor(i / 2) * (2 * rem))
        .attr('font-size', rem)
        .text(d => d[0]);
    }
  }
